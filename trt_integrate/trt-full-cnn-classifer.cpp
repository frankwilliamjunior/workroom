...
#define checkRuntime(op) __check_cuda_runtime((op),code,__FILE__,__LINE__);

bool __check_cuda_runtime(const char* op,cudaError_t code,const char* file,int line){
    if(code != cudaSuccess){
...
inline const char* severity_string(nvinfer1::ILogger::Severity t){
    swithc(t){
        ...
        case nvinger1::ILogger::Severity::kERROR:   return "error";
...
shared_ptr<_T> make_shared(_T* ptr){
...
void inference(){
    ...
    if(engine == nullptr){
        printf("Deserialized cuda engine failed.\n");
        runtime->destroy();
        return;
    }
